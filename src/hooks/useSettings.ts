  ...
  useEffect(() => {
    ...
    root.style.setProperty(
      ...
      settings.calendarSelectionColor === 'auto'
        ? primaryHsl
        : hexToHsl(selectionColor || '#3b82f6')
    );
    ...
    if (settings.fontFamily === 'custom' && settings.customFontUrl) {
      ...
      if (settings.customFontName) {
        document.fonts
          .load(`16px "${settings.customFontName}"`)
          .then(() => {
            document.body.style.fontFamily = customFontFamily;
          })
          .catch(() => {
    ...
    if (Capacitor.isNativePlatform()) {
      widgetsBridge.setWidgetThemeColor(settings.widgetThemeColor || defaultSettings.widgetThemeColor);
    }
  }, [settings]);
